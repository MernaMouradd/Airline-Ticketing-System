				...
				<table class="table table-bordered" id="flight-list">
					...
					<tbody>
						<?php
							...
							while($row2 = $airport->fetch_assoc()){
								$aname[$row2['airport_id']] = ucwords($row2['airport'].', '.$row2['location']);
							}
							$i=1;
							$qry = $conn->query("SELECT b.*,f.*,a.airlines,a.logo_path,b.id as bid FROM  booked_flight b inner join flight_details f on f.flight_id = b.flight_id inner join airlines_list a on f.airline_id = a.airline_id  order by b.id desc");
							//$qry = $conn->query("SELECT b.*,f.*,a.airlines,a.logo_path,b.id as bid FROM  booked_flight b inner join flight_details f on f.flight_id = b.flight_id inner join airlines_list a on f.airline_id = a.airline_id  order by b.id desc");
						 ...
						 <tr>
						 	...
						 	<td>
						 		...
						 		<p><small>Password Number :<b><?php echo $row['passport_no'] ?></small></b></p>
						 	</td>
						 	<td>
						 		<div class="row">
						 		<div class="col-sm-4">
						 		...
						 		<p><small>Departure :<b><?php echo date('M d,Y h:i A',strtotime($row['departure_date'])) ?></small></b></p>
						 		<p><small>Arrival :<b><?php echo date('M d,Y h:i A',strtotime($row['arrival_date'])) ?></small></b></p>
						 		</div>
						 		</div>
						 	</td>
						 	<td class="text-center">
						 			...
						 			<!--<button class="btn btn-outline-danger btn-sm delete_booked" type="button" data-id=" /*echo $row['bid'] ?>"><i class="fa fa-trash"></i></button> -->
						 	</td>
